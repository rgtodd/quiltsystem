...
QUILT.design = {
    activeDesign: null, // Design_DesignData
    designInfo: null, // Design_DesignInfoData
    bodyHeight: null,
    bodyWidth: null,
    canvasPadding: 20,
    canvasSize: 50,
...
function _design_createDesignBlock( /* XDesign_Block */ block) /* XDesign_DesignBlock */ {
    var designBlock = {
        ...
        fabricStyleCount: block.fabricStyleCount,
        fabricStyles: []
    };
...
function _design_createToolbarBlock(index, /* XDesign_Block */ block) {
    ...
    if (block !== null) {
        ...
        divColors.html("");
        var idx = 0;
        for (idx = 0; idx < block.fabricStyleCount; ++idx) {
...
function _design_onLoadDesignComplete( /* Design_DesignData */ response) { // design_loadDesign done handler
...
function _design_onRefreshComplete( /* Design_DesignInfoData */ response) { // design_refresh done handler
    var idx,
        blockInfo;
...
function _design_onBlockListSelection(selection, block /* XDesign_Block */) { // blockList_open callback
    ...
    if (index < QUILT.design.activeDesign.blocks.length) {
        var currentBlock = QUILT.design.activeDesign.blocks[index];
        if (currentBlock !== null) {
            for (var idx = 0; idx < currentBlock.fabricStyles.length; ++idx) {
