...
QUILT.blockList = {
    blockInfos: null, // Design_BlockInfoData[]
...
function _blockList_onRefreshListComplete( /* Design_BlockInfoData[] */ response) {
    QUILT.blockList.blockInfos = response;
...
function _blockList_lookupInfo(blockName) {
    for (var idx = 0; idx < QUILT.blockList.blockInfos.length; ++idx) {
        var blockInfo = QUILT.blockList.blockInfos[idx];
...
function _blockList_populateList() {
    ...
    for (var idx = 0; idx < QUILT.blockList.blockInfos.length; ++idx) {
        ...
        if (blockInfo.category !== currentCategory) {
            currentCategory = blockInfo.category;
            $("#divBlocks").append("<div class='tooltray__heading'>" + currentCategory + "</div>");
